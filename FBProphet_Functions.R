...
globalVariables(c(
  ...
  "monthly", "monthly_lower", "monthly_upper","quarterly", "quarterly_lower", "quarterly_upper",
...
prophet <- function(df = df,
                    ...
                    monthly.seasonality = 'auto',
                    quarterly.seasonality = 'auto',
                    holidays = NULL,
...
validate_inputs <- function(m) {
  ...
  if (!is.null(m$holidays)) {
    ...
    for (h in unique(m$holidays$holiday)) {
      ...
      if (h %in% c('zeros', 'yearly', 'weekly','monthly','quarterly', 'yhat', 'seasonal', 'trend')) {
...
make_all_seasonality_features <- function(m, df) {
  ...
  if (m$monthly.seasonality) {
    seasonal.features <- cbind(
      seasonal.features,
      make_seasonality_features(df$ds, 30.4, 5, 'monthly'))
  }
  if (m$yearly.seasonality) {
    seasonal.features <- cbind(
      seasonal.features,
      make_seasonality_features(df$ds, 91.3125, 7, 'quarterly'))
  }
  if(!is.null(m$holidays)) {
...
set_auto_seasonalities <- function(m) {
  ...
  if (tolower(m$yearly.seasonality) == 'auto') {
    if (last - first < 730) {
  ...
  if (tolower(m$weekly.seasonality) == 'auto') {
  ...
  if (tolower(m$monthly.seasonality) == 'auto') {
    dt <- diff(m$history$ds)
    min.dt <- min(dt[dt > 0])
    if ((last - first < 60.8) || (min.dt >= 30.4)) {
      warning('Disabling monthly seasonality. ',
              'Run prophet with `monthly.seasonality=TRUE` to override this.')
      m$monthly.seasonality <- FALSE
    } else {
      m$monthly.seasonality <- TRUE
    }
  }
  
  if (tolower(m$quarterly.seasonality) == 'auto') {
    dt <- diff(m$history$ds)
    min.dt <- min(dt[dt > 0])
    if ((last - first < 182.625) || (min.dt >= 91.3125)) {
      warning('Disabling quarterly seasonality. ',
              'Run prophet with `quarterly.seasonality=TRUE` to override this.')
      m$quarterly.seasonality <- FALSE
    } else {
      m$quarterly.seasonality <- TRUE
    }
  }
...
prophet_plot_components <- function(
  m, fcst, uncertainty = TRUE, plot_cap = TRUE, weekly_start = 0,monthly_start = 0,
  quarterly_start = 0,yearly_start = 0) {
  ...
  # Plot monthly seasonality, if present
  if ("monthly" %in% colnames(df)) {
    panels[[length(panels) + 1]] <- plot_monthly(m, uncertainty, monthly_start)
  }
  
  # Plot quarterly seasonality, if present
  if ("quarterly" %in% colnames(df)) {
    panels[[length(panels) + 1]] <- plot_quarterly(m, uncertainty, quarterly_start)
  }
...
plot_monthly <- function(m, uncertainty = TRUE, monthly_start = 0) {
  # Compute weekly seasonality for a 1-30 sequence of dates.
  df.m <- data.frame(
    ds=seq.Date(zoo::as.Date('2017-01-01'), by='d', length.out=30) +
      monthly_start, cap=1.)
  df.m <- setup_dataframe(m, df.m)$df
  seas <- predict_seasonal_components(m, df.m)
  seas$ds <- df.m$ds
  
  gg.monthly <- ggplot2::ggplot(seas, ggplot2::aes(x = ds, y = monthly,
                                                  group = 1)) +
    ggplot2::geom_line(color = "#0072B2", na.rm = TRUE) +
    ggplot2::labs(x = "Day of month")
  if (uncertainty) {
    gg.monthly <- gg.monthly +
      ggplot2::geom_ribbon(ggplot2::aes(ymin = monthly_lower,
                                        ymax = monthly_upper),
                           alpha = 0.2,
                           fill = "#0072B2",
                           na.rm = TRUE)
...
plot_quarterly <- function(m, uncertainty = TRUE, quarterly_start = 0) {
  # Compute weekly seasonality for a 1-91 sequence of dates.
  df.q <- data.frame(
    ds=seq.Date(zoo::as.Date('2017-01-01'), by='d', length.out=91) +
      quarterly_start, cap=1.)
  df.q <- setup_dataframe(m, df.q)$df
  seas <- predict_seasonal_components(m, df.q)
  seas$ds <- df.q$ds
  
  gg.quarterly <- ggplot2::ggplot(seas, ggplot2::aes(x = ds, y = quarterly,
                                                   group = 1)) +
    ggplot2::geom_line(color = "#0072B2", na.rm = TRUE) +
    ggplot2::labs(x = "Day of quarter")
  if (uncertainty) {
    gg.quarterly <- gg.quarterly +
      ggplot2::geom_ribbon(ggplot2::aes(ymin = quarterly_lower,
                                        ymax = quarterly_upper),
                           alpha = 0.2,
                           fill = "#0072B2",
                           na.rm = TRUE)
  }
